...
def test_admin_can_create_product(authenticated_admin_client, product_data):
    response = authenticated_admin_client.post(PRODUCTS_LIST_URL, product_data, format='json')
...
def test_get_product_detail_is_not_allowed_for_unauthenticated_user(api_client, db, product_data):
    product = Product.objects.create(**product_data)
    url = reverse("product-detail", args=[product.pk])
