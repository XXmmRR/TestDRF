...
def user_data(db, django_user_model):
    user = django_user_model.objects.create_user(
        email="testuser_for_order@example.com", password="password123"
    )
    admin_user = django_user_model.objects.create_superuser(
        email="admin_for_order@example.com", password="password123"
    )
...
def order_in_db(db, user_data, product_in_db):
    ...
    OrderItem.objects.create(
        order=order, product=product_in_db, quantity=2, price=100.00
    )
    return order
...
def test_authenticated_user_can_list_their_orders(
    authenticated_user_client, order_in_db, db
):
...
def test_authenticated_user_can_create_order(
    authenticated_user_client, order_data, user_data, db
):
    user, _ = user_data
    response = authenticated_user_client.post(
        ORDERS_LIST_URL, order_data, format="json"
    )
...
def test_admin_can_list_all_orders(authenticated_admin_client, order_in_db, db):
    user2 = User.objects.create(
        email="user2_for_order_test@example.com", is_staff=False, is_superuser=False
    )
...
@pytest.mark.django_db
def test_order_creation_sends_confirmation_email(
    authenticated_user_client, order_data, user_data
):
    user, _ = user_data
